...
<body>
  <div class="container">
    ...
    <!-- تاريخ الحجز مع نص توضيحي -->
    <div class="date-wrapper">
      <input type="date" id="date" required oninput="toggleDatePlaceholder()" />
      <span id="datePlaceholder" class="date-placeholder-text">حدد تاريخ الحجز</span>
    </div>
...
${services.join("\n")}`;
    ...
    function toggleDatePlaceholder() {
      ...
      if (dateInput.value) {
        placeholder.classList.add("date-hidden");
      } else {
        placeholder.classList.remove("date-hidden");
      }
    }

    // تشغيل placeholder أول مرة
    document.addEventListener("DOMContentLoaded", toggleDatePlaceholder);
  </script>
</body>
</html>
